...
class Score(State.State):
    ...
    def __init__(self):
        ...
        title_font = PF.Font("Red October-Regular.ttf", 42)
        ...
        self.score_surf = []
        for score in scores:
            self.score_surf.append(main_font.render(score.rstrip(), True, color))

    def render(self):
        G.Globals.SCREEN.fill(PC.Color("black"))
        width, height = self.title_surf.get_size()
        G.Globals.SCREEN.blit(self.title_surf, (G.Globals.WIDTH/2 - width/2, height/2))
        c_height = 2.5*height
        for score in self.score_surf:
            width, height = score.get_size()
            G.Globals.SCREEN.blit(score, (G.Globals.WIDTH/2 - width/2, c_height))
            c_height += height * 1.5
