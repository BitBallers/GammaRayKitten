...
class Menu(State.State):
    ...
    def __init__(self):
        ...
        if not Menu.FONT: # if no font is loaded, load a font
            Menu.FONT = PF.Font("fonts/mrsmonster.ttf", 30)
        if not Menu.TITLEFONT: # if no font for title is loaded, load a title font
            Menu.TITLEFONT = PF.Font("fonts/GUEVARA.ttf", 70)
        if not Menu.IMAGE:
    ...
    def render(self):
        ...
        G.Globals.SCREEN.blit(self.title_surf, (80, 0)) # blit the title surface to the screen
