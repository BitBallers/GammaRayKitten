...
class Map(object):
    ...
    def __init__(self, size):
        ...
        Map.HEIGHT = len(map_matrix)*Map.SUB_HEIGHT
        Map.WIDTH = len(map_matrix[0])*Map.SUB_WIDTH

        # fill in map
        for k in range(size):
            x = 7*50+k*Map.SUB_WIDTH
            y = 0
    ...
    def load_sub_map(self, sub_matrix, row_number, col_number):
        for y_index, row in enumerate(sub_matrix):
            for x_index, char in enumerate(row):
                x = x_index*Map.TILE_WIDTH+col_number*Map.SUB_WIDTH
                y = y_index*Map.TILE_HEIGHT+row_number*Map.SUB_HEIGHT
